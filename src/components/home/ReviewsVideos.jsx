...
export default function ReviewsVideos() {
  ...
  return (
    <section className="py-8 relative overflow-hidden">
      ...
      >
        <h2 className="text-[clamp(1.5rem,5vw,3rem)] font-bold leading-tight text-primary md:text-[clamp(1.25rem,4vw,2.5rem)]">
          {t("title")}
        </h2>
        <p className="text-[clamp(1.25rem,4vw,2.2rem)] font-bold text-secondary mt-2 md:text-[clamp(0.9rem,2vw,1.1rem)] md:font-normal">
          {t("subtitle")}
        </p>
      </motion.div>
      ...
      {guaranteesList.length > 0 && (
        <div className="container mx-auto px-4 sm:px-6 mt-10">
          <h2 className="text-[clamp(1.5rem,5vw,3rem)] font-bold mb-6 text-primary text-center md:text-[clamp(1.4rem,4vw,2.5rem)]">
          ...
          <div className="bg-black/90 backdrop-blur-sm rounded-2xl p-6 border border-[#F47A1F]/20">
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-5">
              {guaranteesList.map((g, i) => (
                <div key={i} className="transition-transform hover:scale-102">
                  <h4 className="text-[clamp(1.25rem,4vw,2.2rem)] font-bold text-[#F47A1F] mb-3 flex items-center gap-2 md:text-sm md:font-black">
                  ...
                  <ul className="space-y-2 pr-4">
                    {g.items?.map((item, idx) => (
                      <li key={idx} className="flex gap-2 text-[#B6BDD6] items-start">
                        ...
                        <span className="font-bold text-[clamp(1.25rem,4vw,2.2rem)] text-white md:text-xs">{item}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              ))}
              <div className="col-span-2 lg:col-span-1 bg-emerald-500/50 backdrop-blur-lg rounded-xl p-5 border border-white/20 flex items-center justify-center transition-transform hover:scale-102 shadow-lg">
                <p className="text-white font-bold text-[clamp(1.25rem,4vw,2.2rem)] text-center leading-relaxed md:text-sm">
